...
const getSafeBalance = (value) => Math.max(0, Number(value || 0));

const canRedeemLoyalty = (user) =>
  getSafeBalance(user?.loyaltyPoints) >= LOYALTY_RULES.REDEEM_POINTS_REQUIRED;

const getLoyaltyDiscountAmount = (totalPrice) => {
...
const createLoyaltyTransaction = async ({
  ...
  const currentBalance = getSafeBalance(user?.loyaltyPoints);
...
export const initiateKhaltiPayment = async (req, res) => {
  try {
    ...
    const isAdvancePayment = paymentType === "advance_30";

    if (!isFullPayment && !isAdvancePayment) {
    ...
    if (isFullPayment && useLoyaltyPoints === true) {
      if (!canRedeemLoyalty(user)) {
        return res.status(400).json({
          message: "You need at least 1000 loyalty points to redeem this reward",
        });
      }

      if (booking.loyaltyRedeemed) {
        return res.status(400).json({
          message: "Loyalty points have already been redeemed for this booking",
        });
      }

      discountFromPoints = getLoyaltyDiscountAmount(totalPrice);
      expectedAmount = Math.max(0, totalPrice - discountFromPoints);
      pointsRedeemed = LOYALTY_RULES.REDEEM_POINTS_REQUIRED;
    }
    ...
    booking.paymentPreference = isAdvancePayment ? "advance_30" : "full";
    booking.discountFromPoints = discountFromPoints;
...
export const verifyKhaltiPayment = async (req, res) => {
  try {
    ...
    const amountPaid = isFullPayment
      ? Math.max(0, totalPrice - Number(booking.discountFromPoints || 0))
    ...
    booking.khaltiPidx = pidx;
    booking.paidAt = new Date();
    booking.paymentStatusLabel = isFullPayment
      ? "Full amount paid"
      : "30% paid";

    let pointsEarned = Number(booking.pointsEarned || 0);

    // Redeem once
    if (
      isFullPayment &&
      Number(booking.pointsRedeemed || 0) > 0 &&
      booking.loyaltyRedeemed !== true
    ) {
      await createLoyaltyTransaction({
        user,
        booking,
        type: "redeem",
        points: Number(booking.pointsRedeemed || 0),
        direction: "debit",
        amountValue: Number(booking.discountFromPoints || 0),
        description: "Redeemed 1000 loyalty points for 60% discount",
      });

      booking.loyaltyRedeemed = true;
    }

    // Earn once only for full payment
    if (isFullPayment && booking.loyaltyAwarded !== true) {
      pointsEarned = LOYALTY_RULES.FULL_PAYMENT_REWARD;
      ...
      await createLoyaltyTransaction({
        ...
        direction: "credit",
        amountValue: amountPaid,
        description: "Earned 100 loyalty points from full payment booking",
      });

      booking.pointsEarned = pointsEarned;
      booking.loyaltyAwarded = true;
    }
    ...
    return res.status(200).json({
      ...
      loyalty: {
        ...
        discountFromPoints: Number(booking.discountFromPoints || 0),
...
export const downloadInvoicePdf = async (req, res) => {
  try {
    ...
    const verifyUrl = `${BACKEND_URL}/api/payment/invoice/${booking._id}/verify?v=${
      booking.updatedAt?.getTime?.() || Date.now()
    }`;
