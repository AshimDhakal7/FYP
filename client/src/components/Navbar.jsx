...
export default function Navbar() {
  ...
  // ✅ auth should be based on token (not just "user" existing)
  const token =
  ...
  // ✅ show Profile/Logout ONLY when logged in AND not on auth pages
      ...
      <div className="mx-auto max-w-6xl px-4 sm:px-6">
        <div className="flex h-16 items-center justify-between">
          ...
          <div className="hidden items-center gap-3 md:flex">
            {showAuthedControls ? (
              <>
                {/* ✅ Profile button (navigate instead of Link) */}
                <button
                  type="button"
                  onClick={() => navigate("/profile")}
                  className="inline-flex h-10 w-10 items-center justify-center rounded-full bg-green-700 text-white text-sm font-extrabold shadow-sm hover:bg-green-800 transition"
                  title="Profile"
                >
                  {initials}
                </button>
